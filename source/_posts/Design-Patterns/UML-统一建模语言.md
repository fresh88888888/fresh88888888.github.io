...
订单经理必须为产品创建新的激活密钥，而客户可以创建并交付`C2V`文件（“计算机指纹”）。一旦新产品密钥和`C2V`文件可供客户服务使用，它就可以激活产品、生成`V2C`文件并将其返回给客户。
客户应用收到的许可证，从而激活安装的试用产品以成为完整产品。通过`HASP SL`许可证和保护密钥，可以防止该产品复制到其他计算机或虚拟机。

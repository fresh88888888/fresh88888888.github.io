...
{% asset_img closure-convert-to-fn-failed.PNG closure convert to fn failed %}
...
```
{% asset_img string-not-implement-copy.PNG String not implement Copy %}
...
{% asset_img drop_trait.png 实现Drop %}
...
{% asset_img vec-over-panic.png 数组越界panic %}
...
{% asset_img recursive-list.png 错误递归类型 %}
...
{% asset_img rct-example.svg Rc<T>共享数据 %}
...
```
{% asset_img rct-example-failed.png Rc<T>Box例子失败 %}
...
{% asset_img refcell-mock-failed.png 编译失败 %}
...
{% asset_img refcell-mock-failed-again.png 编译失败 %}
...
{% asset_img refcell-mock-pass.png 测试通过 %}
...
```
{% asset_img rct-and-refcellt.png 一处修改处处改 %}

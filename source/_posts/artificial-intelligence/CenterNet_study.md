---
title: 姿态估计/目标检测 — CenterNet（PyTorch）
date: 2024-04-09 14:45:11
tags:
  - AI
categories:
  - 人工智能
...
该架构将对象的中心预测为热图。它将预测框的大小作为回归任务。
{% asset_img cn_1.png %}
<!-- more -->
它也用于姿态估计：
{% asset_img cn_2.png %}

人体关节的坐标也可以使用回归来预测。我用这个想法来预测车辆的`x、y、z`坐标以及偏航角、`pitch_cos、pitch_sin、roll`角。对于`pitch`，我预测`sin`和`cos`，因为正如我们看到的，这个角度既可以接近`0`，也可以接近`3.14`。这`7`个参数是我的回归目标变量，而不是`shift_x、shift_y、size_x、size_y`。
...
**`PredictionString`**列包含所有汽车的姿态信息。从数据描述来看：主要数据是汽车图像和相关**姿态信息**。姿态信息被格式化为字符串，如下：`model type, yaw, pitch, roll, x, y, z`。照片中有两辆车的具体示例：`5 0.5 0.5 0.5 0.0 0.0 0.0 32 0.25 0.25 0.25 0.5 0.4 0.7`，我们需要一个函数来提取这些值：
```python
...
我猜想，在这个数据集中，俯仰和偏航是混合在一起的。间距不可能那么大。这意味着汽车是颠倒的。
```python
def rotate(x, angle):
    x = x + angle
...
`x`是从左到右测量的，我认为`y`是距汽车的距离，`z`是路面上方的高度。让我们看看这些点在一个平面上的分布情况如何，尝试在已知`x、z`的情况下预测 `y`：
```python
...
```bash
	ImageId	PredictionString
0	ID_18fb86d04	0.14394063 0.021658449556661755 -3.07686785061...
1	ID_ce06d4dc1	0.15193772 0.027584688562316634 -3.08121419523...
2	ID_afe07078f	0.1380412 -3.120722459673391 -3.11551375326747...
3	ID_37caf75aa	0.15178935 0.011924028786011396 -3.10641848494...
4	ID_7655363ca	0.13959816 -3.1307816952173844 -3.117394277947...
```

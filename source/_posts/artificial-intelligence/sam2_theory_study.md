...
**可提示的视觉分割**(`PVS`)任务可以看作是分割静态图像到视频的扩展。在`PVS`设置中，给定一个输入视频，模型可以对视频中的任何帧进行不同类型的输入（包括点击、框或蒙版）的交互提示，目的是分割（和跟踪）整个视频中的有效对象。与视频交互时，模型会对提示的帧提供即时响应（类似于`SAM`在图像上的交互式体验），并且实时地返回整个视频中对象的分割。
{% asset_img s_7.png "可提示的视觉分割任务(PVS)的图示。例如“分割任何事物”(SA)和半监督视频对象分割(VOS)，可以看作是PVS任务的特殊案例。" %}

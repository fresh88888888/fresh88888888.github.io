...
在本例中，利用数据科学技能帮助识别`Breakthrough Listen`目标扫描中的异常信号。由于没有已确认的外星信号，训练机器学习算法，因此在如大海捞针般的望远镜数据中加入了一些模拟信号（称之为“针”）。目前已经识别出一些隐藏的针，以便您可以训练模型找到更多隐藏的”针“。数据由二维数组组成，因此计算机视觉、数字信号处理、异常检测等方法可能很有优势。
<!-- more -->

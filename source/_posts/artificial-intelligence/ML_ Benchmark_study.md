...
- **召回率**：也称为敏感度，量化了真实阳性（即实际正确的预测）的数量。
- **F1分数**：将**准确率**和**召回率**融合为一个指标。它认为这两个指标具有同等权重，以平衡任何假阳性或假阴性。`F1`分数的范围从`[0,1]`，其中`1`示出色的召回率和准确率。
- **完全匹配**：是`LLM`预测完全匹配的比例，是翻译和问答的标准。
- **BLEU分数**(`BLEU Score`)：衡量`LLM`生成的文本与人工编写的参考文献的匹配程度。这对于翻译和创意写作等任务很重要。
- **ROUGE分数**：以回忆为导向的摘要评估，有几种类型。例如，`ROUGE-N`对摘要进行与BLEU类似的计算，而`ROUGE-L`计算预测摘要和人工生成的摘要之间的最长公共子序列。

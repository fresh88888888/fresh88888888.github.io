...
**向量**是只有一列的数组。事实证明，向量可以看作是平面或高维空间中的箭头。向量的两个非常重要的组成部分是**大小**和**方向**。向量只是一个**元组**。它可以有{% mathjax %}1,2,\ldots,n{% endmathjax %}个值。向量中的坐标值是它离开的空间的维度。例如，坐标`(4, 3)`的向量位于平面中，并且正是指向水平坐标为`4`和垂直坐标为`3`的点的箭头。您可以在更高维度的空间中拥有向量。例如，在`3D`中，坐标为`(4, 3, 1)`的向量位于空间中，并且是指向相对于`x、y`和`z`轴的坐标`(4, 3, 1)`的点的箭头。
{% asset_img m_38.png %}

有许多表示向量的符号，让我们展示一个为{% mathjax %}x{% endmathjax %}的向量。向量可以水平写为**行向量**，也可以垂直写为**列向量**。向量的分量用下标编号。{% mathjax %}x{% endmathjax %}中的第二个分量称为 {% mathjax %}x_2{% endmathjax %}。在其他资源中，您可能会看到向量名称用小箭头或粗体字书写。向量也可以用方括号而不是圆括号来书写。方括号有时可以提醒人们，向量是矩阵的一部分，或者只是一个瘦矩阵。但这些符号之间绝对没有概念上的区别。
{% asset_img m_39.png %}

让我们使用该向量符号将`L1`和`L2`范数的定义推广到具有`n`个总分量的任何向量。一般而言，`L1`范数是所有分量绝对值的总和。`L2`范数是向量所有平方分量总和的平方根。
{% asset_img m_40.png %}

##### Operator操作

就像数字可以相加或相减得到其他数字一样，**向量**也可以相加或相减得到其他向量。这可以通过非常自然的方式完成。如果你想要添加两个向量，例如向量{% mathjax %}u{% endmathjax %}的坐标为`(4, 1)`，向量{% mathjax %}v{% endmathjax %}的坐标为`(1, 3)`，你只需将坐标相加即可得到坐标为`(5, 4)`的向量。这是一个很好的几何解释。恰好是向量{% mathjax %}u{% endmathjax %}和{% mathjax %}v{% endmathjax %}形成的平行四边形的对角线。差值也会发生类似情况，只是平行四边形的另一条对角线不同。
{% asset_img m_41.png %}

向量`(1, 3)`和`(4, 1)`之间的差值也按分量取为`(3, -2)`。也就是说，这个向量看起来并没有什么特别之处，除非你平移它，它与通过连接点`(1, 3)`和`(4, 1)`获得的向量精确匹配。这是向量和与差的一般定义。
{% asset_img m_42.png %}

假设有两个向量{% mathjax %}x{% endmathjax %}和{% mathjax %}y{% endmathjax %}。**和**可以表示为每个分量的和。这意味着新向量的第一个分量将是{% mathjax %}x{% endmathjax %}的第一个分量与{% mathjax %}y{% endmathjax %}的第一个分量的和，依此类推。请注意，按照这个定义，{% mathjax %}x{% endmathjax %}和{% mathjax %}y{% endmathjax %}必须具有相同数量的分量。类似地，两个向量之间的差异定义为分量{% mathjax %}y{% endmathjax %}的差异。两个向量之间的差异有助于判断两个向量之间的距离。
{% asset_img m_43.png %}

例如，向量`(1, 5)`与向量`(6, 2)`有多大差异？一种判断方法是通过它们差异的`L1`范数。这是分量绝对值的总和(`8`)。另一种判断方法是通过它们差异的`L2`范数。
{% asset_img m_44.png %}

在机器学习中，了解向量之间的距离非常有用，因为很多时候你想计算数据点之间的不同相似性，而这些度量非常有用。您可以在向量中执行的另一个非常有用且简单的操作是将它们乘以标量。例如，如果向量为`(1, 2)`，并且您想将其乘以标量{% mathjax %}\lambda = 3{% endmathjax %}，则结果为元素{% mathjax %}y{% endmathjax %}的乘积。这意味着您将向量的每个元素乘以`3`以获得向量`(3, 6)`。从图形上讲，意味着将向量`(1, 2)`拉伸`3`倍。
{% asset_img m_45.png %}

如果标量为负怎么办？这里发生的情况是，向​​量再次被拉伸，但也会反射原点。例如，如果向量再次为`(1, 2)`且标量{% mathjax %}\lambda = -2{% endmathjax %}，则标量与向量的乘积为`(-2, -4)`。这也符合预期，与逐项乘积相对应。
{% asset_img m_46.png %}

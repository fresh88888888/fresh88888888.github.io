...
{% mathjax '{"conversion":{"em":14}}' %}
\Bigg(& n \\& 1 \Bigg) \frac{1}{n}(1 - \frac{1}{n})^{n - 1} = \frac{n}{n - 1}(1 - \frac{1}{n})^n \approx 0.37
{% endmathjax %}
...
减轻这些限制的方法是使用足够快的CPU缓存层次结构来为处理器提供数据。这是深度学习中批量处理背后的推动力。举一个简单的例子：矩阵-矩阵乘法。比如{% mathjax %}\mathbf{A} = \mathbf{BC}{% endmathjax %}，我们有很多方法来计算{% mathjax %}\mathbf{A}{% endmathjax %}。例如，我们可以尝试以下方法：

...
##### 论文解读——未来5年内AI将替代高级软件开发人员

这篇论文[`Measuring AI Ability to Complete Long Tasks`](https://arxiv.org/pdf/2503.14499)，主要提供了评估人工智能在处理需要较长时间任务能力的一种方法。尽管人工智能在基准测试上取得了快速进展，但基准测试的现实意义仍然不明确。为了从人类能力的角度量化人工智能系统的能力，作者提出了一种新的度量标准：`50%`**任务完成时间窗口**。这是指人类通常完成任务所需的时间，而人工智能模型能够以`50%`的成功率完成这些任务。首先测量了具有相关领域专业知识的人类在`RE-Bench、HCAST`以及`SWAA-Suite`共`66`个新的较短任务组合上的完成时间。在这些任务上，当前人工智能模型如`Claude 3.7 Sonnet`的`50%`时间窗口大约为`50`分钟。此外，自`2019`年以来，前沿人工智能的时间窗口大约每七个月翻一番，尽管这一趋势可能在`2024`年加速。人工智能模型时间窗口的增加似乎主要是由于更高的可靠性和纠正错误的能力，结合更好的**逻辑推理**和**工具使用能力**。如果这些结果能够推广到现实世界的软件开发任务中，那么根据这一趋势的推测，**在五年内，人工智能系统将能够自动化许多需要人类一个月才能完成的软件开发任务**。
{% asset_img ml_2.png "智能体完成任务以50%的成功率，在过去六年中大约每七个月翻一番的速度，这一趋势也预示着在不到十年的时间里，我们将看到AI智能体能独立完成需要人类花费数天或数周才能完成的软件任务" %}

如下图所示，作者先定义了一个`diverse task suite`，这些任务来自于三个数据集：`HCAST、SWAA-Suite`和`RE-Bench`，这三个数据集基本上都是跟`Software`、`Agent`相关。从每个数据集里面抽取了一些`task`，这些`task`所花费的时间都是不一样的，比如说：`HCAST`这一类的`task`所花费的时间为`1`分钟`~30`小时区间范围；`SWAA-Suite`这一类`task`所花费时间为`1~30`秒区间范围；`RE-Bench`这一类`task`所花费的时间大概在`8`个小时。当有了这些数据集之后，那么让人类去解决这些`task`，可能需要`2~3`个小时才能解决。用大语言模型`Agent`去执行同样的`task`，并记录一下它们的成功率，然后就分别以人类的消耗时间和`AI Agent`的成功率做一个图，就可以看到这样一种负相关。然后取成功率为`50%`，也就是是说对于一个任务，模型解决这个任务能取得`50%`的成功率，这个时候对于人类所需要花费的时间是多少？这里作者称这个时间为`horizon length`。最后用`horizon length`最为纵轴，模型的发布时间作为横轴画一个图。
{% asset_img ml_3.png "测量AI Agent 时间窗口的方法" %}

这个图就是模型发布的时间与人类解决这类任务需要花费的时间的关系图，比如早期的`GPT-2`模型以`50%`的成功率，解决人类所需要`1~2`秒解决的任务，随着这个时间的发展，比如到了`2025`年，模型变得越来越复杂，然后它能解决的任务也越来越复杂。比如对于`Claude 3.7`来说，它就能够解决人类需要大概`30~60`分钟才能解决的任务。这篇论文提出的`metric`，就把模型的能力与人类的水平给联系起来了。纵轴的时间虽然表示的是人类解决这个任务所花费的时间，但是它侧面却反应的是这个任务的难易程度。所以上面那个图从另外一个角度也表达了：”随着技术的发展，模型所能解决的任务也是越来越复杂“，这里作者给了一个例子，对于不同的任务，这种复杂的程度到底是什么样子？如下所示：
|任务|花费时间|任务描述|
|--|--|--|
|`find shell script`|3秒|多项选择：那个文件是`shell`脚本？选项：`run.sh，run.txt，run.py，run.md`|
|`wikipedia research`|1分钟|研究维基百科上的简单事实信息，并对简单的问题提供准确的回答。|
|`oxdna simple`|9分钟|检测并修复使用`oxDNA`软件包进行分子动力学模拟的输入文件中的错误。|
|`munge data`|56分钟|编写一个`Python`脚本，通过从提供的示例文件中推断转换规则，将`JSON`数据从一种格式转换为另一种格式。|
|`cuda backtesting`|8小时|通过实现自定义`CUDA`内核，加速`Python`回测工具的交易执行，同时保留所有功能，目标是提高`30`倍性能。|

下面这张图展示的是：这个模型的成功率是多少？从最早期的`GPT-2`模型，一直到现在`GPT4o、Claude 3.5、o1、Claude 3.7`。我们可以看到在这个`benchmark`上面，`Claude 3.7`的性能是最好的，因为它的成功率大概能到`60%`左右。但是它离`100%`还有一定的距离。
{% asset_img ml_4.png "模型的在任务(General Autonomy + SWAA + RE-Bench)上的成功率表现" %}

接下来画了一下模型的成功率与人类需要解决这类任务所花费的时间的关系图，这也是一个负相关的曲线。如下图所示，如果人类在这类任务上花费的时间比较多，说明这类任务是比较难的任务，所以模型模型的成功率就比较低。
{% asset_img ml_5.png "模型的成功率与人类需要解决这类任务所花费的时间的关系图" %}

这里作者也分析了一下，为什么`AI Agent`在一些任务上失败了：
- **糟糕的计划和工具选择**：智能体生成的高层次计划而无法独立完成，或者选择的工具无法完成预期目标。
- **错误的心算或推理**：智能体在关键步骤中进行了错误的心算或逻辑推理，导致任务失败。
- **过早放弃任务**：智能体在任务进行到一半时放弃，提交了无意义的答案或未经检查的解决方案。这些失败通常是由于智能体在查看所有的代码或信息之前提交了结果。
- **重复失败的操作**：智能体重复进行无法推进问题解决的行为，例如反复执行错误的命令，而不尝试其他方法。

当模型推理能力加强了之后，出现`repeating actions`（重复执行）的情况反而减少了。可能跟模型具有一定的思考能力有一定关系吧。接下来，看看下面这张图。作者做了一个实验，并展示了它的实验结果。
{% asset_img ml_6.png  %}

如上图所示，纵轴代表的是估算(`estimation`)的方法，我们就看最后一个”`overall 2024-2025`“，作者这里选的是：把任务的难度外推`1`个月时间长度。这里为什么选择一个月时间长度呢？这里有一个定义”`One month AGI`“，也就是说如果模型能够解决人类需要大概一个月的时间完成的的任务，那么这样的`AI`基本就已经超过了人类在完成一些比较复杂的大型的软件开发项目的能力，或者比如一些新的科学发现。所以作者在这里选了一个`one month horizon`作为`AGI`的代表。我们来看一下如果基于之前的结果进行外推的话，这个`one month AGI`大概能在什么时候实现呢？根据作者的预测，大概在`2027~2028`年前后，这个`one month AGI`就可以达到了。现在是`2025`年，也就是大概还有`2~3`年时间。我们就有望看到一个非常强大的`AI`，能够自主的帮我们完成非常复杂的软件开发项目和帮助我们做科研，这个时间点真的是有可能实现。

下面这张图，作者主要是比较了一下任务的难度，也就是人类完成这些任务大概需要多久。纵坐标表示的是一个`cost ratio`，也就是模型的`cost`与人类的`cost`进行比较。所以这个值越小代表模型的`cost`值越低。可以看到纵坐标最高的点是`1`，所以这下面所有的点都表示模型比人类更便宜。对于那些特别简单的任务，人类大概只需花一分钟这样的一些任务，模型的`cost`和人类的`cost`比是非常低的。大概在`1%`左右，意思就是说人类大概只需要一分钟时间完成的任务，模型的消耗只有人类消耗的`1%`。还有一些任务的`cost`值更低，这就意味着随着`AI`技术的发展，一旦`AI`能解决这一类问题的时候，那么它的成本将是非常低的。目前来看，对于比较复杂的任务，大概是在`1%~10%`之间，那么对于我们人类来说，就没有比较去学习处于被`AI`替代的技能了，我们应该尽量去学习那些更加复杂，`AI`取代不了的技能。这样我们才能在社会中生存而不被`AI`所取代。
{% asset_img ml_7.png  "使用LLM智能体成本占人类专家执行相同任务成本的比例" %}

##### 总结

这篇论文主要是提出了一种`metric`，这种`metric`与一般的`metric`不太一样，就是它把人类的能力考虑了进来。用人类完成任务的时间作为一个`baseline`，然后基于这个`baseline`去评价模型的能力。第二个就是做了一些`evaluation`，比较了当前模型在比较复杂的任务上的性能如何，对于一些比较简单的任务，比如人类只需要花费`1~2`分钟完成的任务，模型已经能解决的比较好了。但比较复杂的任务，模型还有待提高。最后作者做了一个`extraplottion`的实验，预期大概在`2027~2028`年前后，至少在**软件开发项目**这一类任务上能够实现`AGI`。
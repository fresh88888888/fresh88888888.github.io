...
|`oxdna simple`|9分钟|检测并修复使用oxDNA软件包进行分子动力学模拟的输入文件中的错误。|
|`munge data`|56分钟|编写一个Python脚本，通过从提供的示例文件中推断转换规则，将JSON数据从一种格式转换为另一种格式。|
|`cuda backtesting`|8小时|通过实现自定义CUDA内核，加速Python回测工具的交易执行，同时保留所有功能，目标是提高30倍性能。|
...
如上图所示，纵轴代表的是估算(`estimation`)的方法，我们就看最后一个”`overall 2024-2025`“，作者这里选的是：把任务的难度外推1个月时间长度。这里为什么选择一个月时间长度呢？这里有一个定义”`One month AGI`“，也就是说如果模型能够解决人类需要大概一个月的时间完成的的任务，那么这样的AI基本就已经超过了人类在完成一些比较复杂的大型的软件开发项目的能力，或者比如一些新的科学发现。所以作者在这里选了一个`one month horizon`作为`AGI`的代表。我们来看一下如果基于之前的结果进行外推的话，这个`one month AGI`大概能在什么时候实现呢？根据作者的预测，大概在`2027~2028`年前后，这个`one month AGI`就可以达到了。现在是2025年，也就是大概还有2~3年时间。我们就有望看到一个非常强大的`AI`，能够自主的帮我们完成非常复杂的软件开发项目和帮助我们做科研，这个时间点真的是有可能实现。

...
如何区分**多智能体逆向学习**与**单智能体**问题。简而言之，在具有战略性的**智能体**问题中（这些**智能体**并非仅仅是**傀儡**），需要处理：群体中**智能体**偏离（一种新的分布变化）。这种新的分布变化来源于无法通过与环境有效的交互（即**逆向强化学习**）。在这种情况下，需要能够估计专家在**反事实状态**下的行为。基于这一问题，提出了`2`种**最小化遗憾差距**的方法：`MALICE`(在专家覆盖假设下) 和 `BLADES`(在可查询专家的情况下)。

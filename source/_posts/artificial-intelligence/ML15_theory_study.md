...
如果曝光给用户的物品两两之间不相似，就说明推荐的结果具有**多样性**，想要提升多样性，首先想办法度量两个物品有多相似，有`2`种度量相似性的方法。这两种方法都是实际有用的，第一种方法是基于物品属性的标签，比如类目、品牌、关键词等等。2个物品有越多相同的标签，则`2`个物品就越相似。第二种方式是基于物品的向量表征，两个物品向量余弦值越大，则两个物品就越相似。有很多种方法对物品向量表征，但并不是所有向量表征的方法都适用**多样性**的问题。
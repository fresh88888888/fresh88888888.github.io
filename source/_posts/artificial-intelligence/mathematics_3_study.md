...
首先，看一下包含条目 2,1, 0,1,3 的矩阵，看看它对正方形周围这些点的作用。这应该使整个变换清晰可见。如前所述，两个水平向量被拉伸了两个，对角线被拉伸了三个。现在对于其他点，情况也是如此。现在将其与另一个矩阵进行比较，该矩阵只是将整个平面在任何方向上拉伸三倍。该矩阵包含条目 3,0,0 和 3，它实际上是单位矩阵的三倍。请注意，这两个变换不是同一个变换，但它们在许多点上确实重合。换句话说，它们对无限多个点的作用完全相同。这条线上的所有点。更具体地说，在这个对角线上，这两个变换做完全相同的事情。它们在无限多个点上匹配。现在这很奇怪，变换应该只在一个点匹配，即点 0,0。当它们在无限多个点匹配时，就会发生一些非奇异的事情。发生了什么？好吧，让我们看看区别。如果这两个变换在无穷多个点上匹配，这意味着在无穷多个点上的差异为零。如果将这两个矩阵的差异应用于这些对角线上的任意向量，则会得到向量 0,0。换句话说，这个矩阵乘以向量 x、y 的无穷度量向量是 0,0。这就是奇异变换的特征。回想一下，非奇异变换对方程矩阵乘以向量等于 0,0 有一个唯一解，那就是向量 0,0。如果你有无穷多个主解，意味着你的矩阵是奇异的，你可以验证这确实是一个奇异矩阵，因为行列式为零。现在让我们做一些类似的事情，但对于右边的另一个变换。我们的变换与之前完全一样。现在让我们将它与在每个方向上将平面拉伸两倍的变换进行比较。这两个并不相同，但它们在整条线上匹配。换句话说，左边的矩阵乘以 xy 等于右边的矩阵乘以 xy。对于这条线上的任何向量 xy，都有无穷多个点。我们可以执行相同的过程，取差值，并将该矩阵乘以一个向量，对于无穷多个向量，该向量等于 0,0。这意味着矩阵 0,1, 0,1，或者我们的矩阵与两倍恒等矩阵之间的差值，是一个奇异矩阵。您可以检查它确实是奇异矩阵，因为它的行列式为零。特征值有什么特别之处？特征值 2 和特征值 3 发生了什么？让我们从总体上考虑一下。如果 Lambda 是特征值，那么对于无穷多个向量 xy，由矩阵给出的变换和通过将平面完全缩放 Lambda 的因子给出的变换相等。这意味着它们的差乘以一个向量，对于无穷多个向量，等于 0,0，这是一个创建许多解的方程。因此，这个矩阵，2-Lambda，1,0,3 减 Lambda 必须是奇异矩阵，所以我们的行列式是零，当我们展开它时，它的行列式由这个方程给出，这被称为特征多项式。基本上，要找到特征值 Lambda，我们需要做的就是查看特征多项式并找到根。特征多项式为零的地方就是特征值。在这种情况下，它们将是二和三。现在你有了特征值，让我们尝试找到特征向量。回想一下，特征向量是满足方程矩阵乘以向量等于特征值乘以向量的向量。如果我们展开它，我们会得到这些方程，它们的解是 x=1、y=0 或任何多重向量。这是特征值向量之一，对应于特征值 2。我们对三个特征值做同样的事情，求解这些方程并得到 1,1。特征向量 1,1 对应于特征值 3。现在您可以进行测验了。找出此矩阵的特征值和特征向量。答案是，对于特征值，它是 11,1；对于特征向量，它是 2,1，对应于特征值 11；-1,2 对应于特征值 1。为什么？好吧，如果您查看特征多项式，它是包含条目 9-Lambda、4,4 和 3-Lambda 的矩阵。它展开为 Lambda^2-2 Lambda + 11，其因式分解为 Lambda-11*Lambda-1。因此，特征值是 Lambda 等于 11 和 Lambda 等于 1。我将把它留给您作为练习，以求解特征向量的方程并验证它们将是 2,1 和 -1,2 或它们的某个倍数，因为重要的是方向。在三乘三矩阵中查找向量的过程非常相似。考虑 A 等于 2,1,-1, 1,0,-3, -1,-3,0。特征多项式是 A 减 Lambda 乘以任意大小的单位矩阵的行列式。在本例中，Lambda 减 I 将是一个三乘三矩阵，如下所示。特征多项式将是这两个矩阵差的行列式。使用对角线法计算三乘三矩阵的行列式，您可以构造特征多项式的各个部分，然后组合项以得到多项式 -Lambda^3+2 Lambda^2+11 Lambda-12=0。对这个多项式进行因式分解（我不会在这里详细展示）将得到这三个因式。现在很容易找到这个等式的零点，即 Lambda 为 -3、1 或 4 时的零点。这给出了这个矩阵的三个特征值。现在您已经从矩阵 A 中找到了三个特征值，让我们找出与每个特征值相关的特征向量。让我们从最后一个特征值 4 开始。请记住，要找到特征值，您需要求解方程 Av=Lambda v。这意味着您需要求解此方程，其中 x_1、x_2 和 x_3 是特征向量的三个分量。将形式乘以向量可得到新向量 4x_1、4x_2、4x_3。完成左侧的点积可得到这个新向量。现在您需要做的就是将这三个向量设置为相等，然后求解 x_1、x_2 和 x_3。我将在此处重写方程组。现在从右侧减去项，只留下零。这给出了您需要求解的最终方程组，标记为第 1、2 和 3 行。您可以使用此方程求解 x_1、x_2 和 x_3。将第 2 行和第 3 行相加，得到 -7x_1-7_x3=0，